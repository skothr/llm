...
\begin{equation}
  ...
  \label{eq:ffn-relu}
\end{equation}
where $W_1 \in \R^{\dmodel \times \dff}$, $W_2 \in \R^{\dff \times \dmodel}$, and
$\ReLU(z) = \max(0, z)$.
...
\begin{equation}
  ...
  \label{eq:ffn-swiglu}
\end{equation}
where $\otimes$ is element-wise multiplication, $\Swish_\beta(x) = x \cdot \sigma(\beta x)$,
...
comparisons \cite{shazeer2020}, and is used in LLaMA \cite{touvron2023} and PaLM. The
...
\end{equation}
where $\mu = \frac{1}{d}\sum_{k=1}^{d} v_k$ is the mean,
...
Modern LLMs (GPT, LLaMA, Claude, Mistral, etc.) are all \emph{decoder-only} Transformers.
This section traces why and how the field converged on this architecture.
